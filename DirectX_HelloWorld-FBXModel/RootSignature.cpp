...
RootSignature::RootSignature()
{
	...
	CD3DX12_ROOT_PARAMETER rootParam[2] = {};	//定数バッファとテクスチャその2
	...
	desc.NumParameters = std::size(rootParam);	//ルートパラメーターの個数を入れる
	desc.NumStaticSamplers = 1;		//サンプラーの個数を入れる
	desc.pParameters = rootParam;	//ルートパラメーターの個数を入れる
	desc.pStaticSamplers = &sampler;	//サンプラーのポインターを入れる
	...
	hr = g_Engine->Device()->CreateRootSignature(
		0,	//GPUが複数が複数ある場合のノードマスク（今回は1個しか無い想定0）
